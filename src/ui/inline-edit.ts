...
export function openDeleteContactModal(groupName: string, index: number): void {
  const group = groups.find(g => g.name === groupName);
  if (!group || index < 0 || index >= group.contacts.length) return;
  ...
  modal.querySelector('#modal-confirm')?.addEventListener('click', () => {
    ...
    if (document.querySelector('.groups-menu-overlay')) {
      const hide = (window as any).hideGroupsMenu as (() => void) | undefined;
      const show = (window as any).showGroupsMenu as (() => void) | undefined;
      hide?.();
      show?.();
    }
...
export function startInlineEdit(row: HTMLElement, groupName: string, index: number): void {
  const group = groups.find(g => g.name === groupName);
  if (!group || index < 0 || index >= group.contacts.length) return;
  const contact = group.contacts[index]!;
  ...
  const nameSpan = row.querySelector('.contact-name') as HTMLElement | null;
  const numberSpan = row.querySelector('.contact-number') as HTMLElement | null;
  if (!nameSpan || !numberSpan) return;
  ...
  if ((window as any).IMask) {
    (window as any).IMask(numberInput, { mask: '+{7} (000) 000-00-00' });
  } else {
  ...
  const actions = row.querySelector('.contact-actions') as HTMLElement | null;
  let dd: CustomDropdown | undefined;
  if (actions) {
  ...
  const applyChanges = () => {
    ...
    let error = false;
    if (!newName) { markInvalid(nameInput, 'Поле не должно быть пустым'); error = true; } else { clearInvalid(nameInput); }
    if (!newNumberSan || !isValidPhone(newNumberSan)) { markInvalid(numberInput, 'Только цифры, допустимы + в начале и скобки'); error = true; } else { clearInvalid(numberInput); }
    ...
    if (nextGroupName !== groupName) {
      ...
      if (dst) {
        ...
        if (moved) dst.contacts.push(moved);
      }
    }
  ...
  nameInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') { e.preventDefault(); revertChanges(); }
  });
