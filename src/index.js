...
function toggleAddContactMenu() {
    const now = Date.now();
    if (now - lastContactsToggleAt < 250)
        return;
...
function showAddContactMenu() {
    if (document.querySelector('.contacts-menu-overlay'))
        return;
  ...
  `;
    ...
    const ddContainer = overlay.querySelector('#contact-group-dd');
    if (ddContainer) {
    ...
    const numInput = overlay.querySelector('#contact-number');
...
function saveContactFromMenu() {
    const overlay = document.querySelector('.contacts-menu-overlay');
    if (!overlay)
        return;
    const nameInput = overlay.querySelector('#contact-name');
    const numInput = overlay.querySelector('#contact-number');
    const dd = overlay._dd;
...
function startInlineEdit(row, groupName, index) {
    ...
    const saveChanges = () => {
        ...
        if (!newNumber || !isValidPhone(sanitizePhoneInput(newNumber))) {
            markInvalid(numberInput, 'Неверный номер');
            hasError = true;
        }
        else {
...
renderMainGroups();
window.renderMainGroups = renderMainGroups;
window.showGroupsMenu = showGroupsMenu;
window.hideGroupsMenu = hideGroupsMenu;
